    ...
    for cycle_index in range(config.cycle_num):
        ...
        for epoch in range(0, config.cycle_inter):
            ...
            if epoch >= config.cycle_inter // 2:
                net.eval()
                valid_loss, _ = do_valid_test(net, valid_loader, criterion, device)
                net.train()
...
def run_test(config, dir):
    ...
    # net ---------------------------------------
    net = get_model(model_name=config.model, image_size=config.image_size, patch_size=config.patch_size, num_class=2)
    net = torch.nn.DataParallel(net)
    net = net.to(device)
    ...
    valid_loss, out = do_valid_test(net, valid_loader, criterion, device)
